...
The backfill queue is used to improve the overall efficiency of the cluster by utilizing idle resources that would otherwise remain unused. **Only jobs that ran out of hours may use the backfill queue, and the maximum wall-time is 12 hours and is subject to change.** Users can submit jobs to the backfill queue by specifying it as the target queue in the PBS select statement. For example:
